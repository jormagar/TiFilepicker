...
#define TAG "ti.filepicker"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Filepicker_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Filepicker.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::FilepickerBindings::lookupGeneratedInit(
...
static void Filepicker_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Filepicker_getBinding)->GetFunction());
}

static void Filepicker_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::FilepickerBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry FilepickerBinding = {
	"ti.filepicker",
	Filepicker_init,
	Filepicker_dispose
};
...
Java_ti_filepicker_FilepickerBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.filepicker", &FilepickerBinding);
	titanium::KrollBindings::addExternalLookup(&(::FilepickerBindings::lookupGeneratedInit));
}
